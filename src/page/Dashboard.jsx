...
function Dashboard() {
  ...
  const buildStats = (data) => {
    ...
    const totalPrice = data.reduce(
      (sum, e) => sum + Number(e.price_course || 0),
      0,
    );
  ...
  const fetchEnrollments = async () => {
    ...
    try {
      const res = await axios.get(
        `https://laravel-api-enrollmentnew-main-m8wa07.free.laravel.cloud/api/admin/enrollment?search=${search}`,
        {
          headers: {
            ...
            <tbody>
              {loading ? (
                ...
                enrollments.map((enrollment) => (
                  <tr
                    key={`${enrollment.id}-${enrollment.student_id}-${enrollment.course.course_id}`}
                  >
                    {/* Student Name */}
                    ...
                    {/* Term */}
                    <td>{enrollment.termslot.tern_day}</td>

                    {/* Time Slot */}
                    <td>
                      {enrollment.timeslot.start_time} -{" "}
                      {enrollment.timeslot.end_time}
                    </td>
                    ...
                    <td className="d-flex justify-content-center align-content-center pt-3">
                      <div className="d-flex justify-content-start  align-content-center  w-50 ">
                        ...
                        <p className=" btn btn-sm btn-outline-danger small opacity-75 py-0  me-2">
                          Approve
                        </p>
                      </div>
                    </td>
                  ...
                  <div className="modal-body mt-3 mb-0">
                    <div className="row g-3">
                      ...
                      <div className="col-md-6 text-start mt-1">
                        ...
                        <p className="">{selectedEnrollment.email} </p>
                      </div>

                      <div className="col-md-6 text-start ps-5 mt-1">
                        <small className="text-muted">Phone_number</small>
                        <br />
                        <small className=" px-1 py-1 rounded-3 m-0 p-0">
                          {selectedEnrollment.phone}
                        </small>
                      </div>
                      <hr className="mt-0" />
                      <small className="text-muted text-start mt-0 mb-2">
                        Course Informantion
                      </small>
                      <div className="col-md-6 text-start mt-1">
                      ...
                      <div className="col-md-6 text-start ps-5 mt-1">
                        <small className="text-muted">Price_course</small>
                        <br />
                        <small className=" px-1 py-1 rounded-3 m-0 p-0">
                          {selectedEnrollment.price.price_course}.00$
                        </small>
                      </div>
                      <div className="col-md-6 text-start mt-0">
                        <small className="text-muted ">Timeslot </small>
                        <p className="small">
                          {selectedEnrollment.timeslot.start_time} -{" "}
                          {selectedEnrollment.timeslot.end_time}{" "}
                        </p>
                      </div>

                      <div className="col-md-6 text-start ps-5 mt-0">
                        <small className="text-muted">term_slot</small>
                        <br />
                        <small className=" px-1 py-1 small rounded-3 m-0 p-0">
                          {selectedEnrollment.termslot.tern_day}
                        </small>
                      </div>
                      <hr className="mt-0 mb-0" />
                      <div className="col-md-6 text-start mt-1">
                        <small className="text-muted ">Cread_at </small>
                        <p className="">{selectedEnrollment.created_at} </p>
                      </div>
                    </div>
                  </div>
